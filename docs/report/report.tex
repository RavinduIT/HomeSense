...
\subsubsection{Commanding a device that has not reported}

A device created from the application has never been reported on, so its status
is \code{DISCONNECTED}. The control is nonetheless enabled.

Disabling it appears defensible, on the grounds that one cannot operate what one
cannot reach, and it was the original behaviour. It is wrong for two reasons.
The application writes \code{desiredState}, which is an intention rather than an
instruction that has taken effect, and recording an intention for a device that
is currently unreachable is precisely what that field is for: the write is
queued and applied when the device reports back. More decisively, every newly
placed device begins in that state, so refusing commands left each one
permanently unusable with no action available to the user.

What the interface must not do is imply the command took effect, and it does
not. The status continues to read \code{DISCONNECTED}, and the command is
reported as saved rather than confirmed. The reconciliation window described
above is also skipped, since a device that has never reported will not answer
within it and would otherwise produce a misleading failure message.

\subsubsection{Session lifecycle}

The destination is derived from session state rather than navigated to. A gate
above the navigation graph resolves three cases: the persisted session is still
being restored; there is no session; or there is a session but no household, in
which case onboarding is shown. Only when both an account and a household exist
is the dashboard composed, so no screen has to defend against a null session.

Signing out therefore performs no navigation. The session state changes, the
gate recomposes, and the whole graph is discarded together with the view models
holding the previous account's data.

Restoration is distinguished from absence for the same reason. Firebase restores
a persisted session asynchronously, so treating ``not yet known'' as ``signed
out'' would show the sign-in screen briefly on every launch by an already
signed-in user.

\subsection{Verifying the enforcement}

The rules were tested against the deployed project rather than assumed correct.
Table \ref{tab:rulescheck} lists the checks and their outcomes.

\begin{table}[h]
\centering
\caption{Security rules verified against the deployed database}
\label{tab:rulescheck}
\begin{tabular}{@{}L{7.4cm}L{3.4cm}L{3.4cm}@{}}
\toprule
\textbf{Operation} & \textbf{Expected} & \textbf{Observed} \\
\midrule
Read any node without authenticating & Denied & \code{401} \\
Owner creates a household & Permitted & \code{200} \\
Owner reads that household & Permitted & \code{200} \\
A different account reads that household & Denied & \code{401} \\
\bottomrule
\end{tabular}
\end{table}

The fourth row is the one that matters: tenancy is not merely intended but
observed, since a second authenticated account was refused.
...
It authenticates with the same account as the mobile client and then discovers
the households that account belongs to. This follows from the security rules
rather than being a convenience: a household is readable only by its members,
so hardware belonging to a household must itself be a member. An earlier
version signed in anonymously, which stopped working the moment tenancy was
enforced, and correctly so.

The household identifier is generated when the household is created, so it
cannot be configured in advance. It is read from \code{/users/\{uid\}/homes},
the index each account keeps of its own households, which is what permits
discovery without reading anything the account does not own.
...
\midrule
Application unit tests & 77 & Grid mapping, wire format, repository propagation and session teardown, Room aggregation, CSV escaping, schedule windows \\
...
A1  & Accounts, sign-in and sign-out & \code{data/auth/}, \code{ui/account/} & Verified against the deployed project & --- \\
A2  & Household tenancy and roles & \code{database.rules.json} & Table \ref{tab:rulescheck} & --- \\
...
sign in as the same kind of household member, so the security rules cannot tell
one from the other. A modified client could therefore write
\code{reportedState} as though it were hardware. Separating the two roles would
require custom authentication claims and a service to issue them, which is
outside the scope of this assignment. What no client can do is write
\code{status}, which is the property on which the safety argument depends.
